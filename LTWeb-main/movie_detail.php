...
function formatVietnameseDay($dateStr) {
    $days = [
        'Mon' => 'Thứ 2',
        'Tue' => 'Thứ 3',
        'Wed' => 'Thứ 4',
    ...
    $englishDay = date('D', strtotime($dateStr));
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    ...
    <div class="grid grid-cols-1 md:grid-cols-[280px_1fr] gap-8 mb-12">
        <!-- Poster -->
        <div class="rounded-2xl overflow-hidden border border-slate-800 shadow-xl shadow-black/30 h-fit bg-slate-900">
        ...
        <div>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6 text-sm">
                ...
                <div>
                    ...
                    <p class="text-slate-200 font-medium"><?php echo (int)($movie['duration'] ?? 0); ?> phút</p>
                </div>
                <div>
                    <p class="text-slate-500 text-xs uppercase font-semibold mb-1">Thể loại</p>
                    <p class="text-slate-200 font-medium"><?php echo htmlspecialchars($movie['genre'] ?? 'Đang cập nhật'); ?></p>
                </div>
                <div>
            ...
            <div class="flex flex-wrap gap-2 mb-6" id="dateTabs">
                <?php foreach ($dateKeys as $i => $date): ?>
                    <button type="button" data-target="date-panel-<?php echo $i; ?>"
                            class="date-tab-btn px-4 py-2 rounded-xl text-sm font-semibold border transition-all cursor-pointer <?php echo $i === 0 ? 'bg-rose-600 border-rose-600 text-white' : 'bg-slate-900 border-slate-800 text-slate-300 hover:border-rose-500'; ?>">
                        <?php echo date('d/m', strtotime($date)); ?>
                        <span class="block text-[10px] font-normal opacity-80"><?php echo formatVietnameseDay($date); ?></span>
                    </button>
                <?php endforeach; ?>
            </div>
                        ...
                        <div class="bg-slate-900 border border-slate-800 rounded-2xl p-5">
                            ...
                            <div class="flex flex-wrap gap-2.5 ml-6">
                                <?php foreach ($data['slots'] as $slot): ?>
                                    <a href="booking.php?showtime_id=<?php echo (int)$slot['showtime_id']; ?>"
