...
<div class="max-w-xl mx-auto bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-xl my-6">
    ...
    <form method="POST" class="flex gap-2 mb-6">
        <input type="text" name="booking_code" value="<?= htmlspecialchars($_POST['booking_code'] ?? '') ?>" placeholder="Nhập hoặc quét mã vé (VD: CS123456)..." autofocus required class="flex-1 bg-slate-950 border border-slate-800 rounded-lg px-4 py-2.5 text-sm focus:border-rose-500 focus:outline-none">
    ...
    <?php if ($message): ?>
        <div class="mb-4 p-3 bg-emerald-500/20 border border-emerald-500/50 text-emerald-400 text-sm rounded-lg text-center font-bold"><?= htmlspecialchars($message) ?></div>
    <?php endif; ?>
    <?php if ($error): ?>
        <div class="mb-4 p-3 bg-rose-500/20 border border-rose-500/50 text-rose-400 text-sm rounded-lg text-center font-bold"><?= htmlspecialchars($error) ?></div>
    <?php endif; ?>
    ...
    <?php if ($ticket): ?>
        <div class="bg-slate-950 border border-slate-800 rounded-lg p-4 space-y-3">
            <div class="flex justify-between border-b border-slate-800 pb-2">
                ...
                <span class="font-bold text-rose-400"><?= htmlspecialchars($ticket['booking_code']) ?></span>
            </div>
            <div class="flex justify-between border-b border-slate-800 pb-2">
            ...
            <div class="flex justify-between border-b border-slate-800 pb-2">
                ...
                <span class="font-bold text-emerald-400"><?= htmlspecialchars($ticket['seats']) ?></span>
            </div>
            ...
            <?php if ($ticket['status'] !== 'used' && $ticket['status'] !== 'cancelled'): ?>
                <form method="POST" class="mt-4">
                    <input type="hidden" name="booking_code" value="<?= htmlspecialchars($ticket['booking_code']) ?>">
